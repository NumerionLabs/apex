...
class EdgeMessagePassingLayer(nn.Module):
    ...
    def __init__(
        ...
        self.edge_net["node_row_fc0"] = nn.Linear(
        ...
        self.edge_net["fc1"] = nn.Linear(
            self.hidden_dim, self.edge_dim + use_attention
        )
        if self.drop_rate > 0:
    ...
    def edge_model(
        self, node_feats, edge_feats, edge_index, graph_feats, graph_index
    ):
        row, col = edge_index
        ...
        if self.aggregate == "attn":
            edge_feats_, edge_attn_logit = torch.split(
                carry, [self.edge_dim, 1], dim=1
            )
    ...
    def node_model(
        ...
        if self.aggregate == "attn":
            ...
            edge_messages = scatter.add(edge_feats * edge_attn, row, 0)
        else:
            assert edge_attn is None
            if self.aggregate == "max":
                edge_messages = scatter.amax(edge_feats, row, 0)[0]
            elif self.aggregate == "mean":
                edge_messages = scatter.mean(edge_feats, row, 0)
            elif self.aggregate == "sum":
                edge_messages = scatter.add(edge_feats, row, 0)
            else:
                raise ValueError(
                    "aggregate must be set to 'attn', 'max', 'mean', or 'sum'."
                )
        ...
        if self.aggregate == "attn":
            ...
            graph_feats_ = scatter.add(carry * node_attn, graph_index, dim=0)
        else:
            node_feats_, carry = torch.split(
                carry, [self.node_dim, self.graph_dim], dim=1
            )
            if self.aggregate == "max":
                graph_feats_ = scatter.amax(carry, graph_index, dim=0)[0]
            elif self.aggregate == "mean":
                graph_feats_ = scatter.mean(carry, graph_index, dim=0)
            elif self.aggregate == "sum":
                graph_feats_ = scatter.add(carry, graph_index, dim=0)
            else:
                raise Exception(
                    "aggregate must be set to 'attn', 'max', 'mean', or 'sum'."
                )
...
class EdgeMessagePassingNetwork(nn.Module):
    """
    Edge message passing graph neural network.
    """
