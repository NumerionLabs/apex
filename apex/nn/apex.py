...
class APEXFactorizedCSL(nn.Module):
    ...
    def _get_checkpoint_state(
        self, iteration: int, loss: float, optimizer: torch.optim.Optimizer
    ) -> dict:

        return {
            self.ITER_STATE_KEY: iteration,
            self.MODEL_STATE_KEY: self.factorizer.state_dict(),
            self.OPTIMIZER_STATE_KEY: optimizer.state_dict(),
            self.LOSS_STATE_KEY: loss,
        }
    ...
    def fit(
        ...
        # Unpack config
        device = config.get("device", next(self.factorizer.parameters()).device)
        ...
        optimizer = optimizer_cls(
            self.factorizer.parameters(),
            **optimizer_kwargs,
        )

        # Instantiate dataloader
        dataloader = (
            self.dataset.create_dataloader_stratified_uniformly_at_random(
        ...
        if state is not None and len(state) > 0:
            optimizer.load_state_dict(state[self.OPTIMIZER_STATE_KEY])
            self.factorizer.load_state_dict(state[self.MODEL_STATE_KEY])
            checkpoint_iter = state[self.ITER_STATE_KEY]
            for _ in range(checkpoint_iter):
                next(data_iterator)

        # Start training
        logger.info(
            f"Beginning training APEX for {max_iterations:,} iterations."
        )
        (sum_loss, ct) = (0, 0)
        for iteration, batch in enumerate(dataloader):
            ...
            block_synthon2rgroup = torch.stack(
                [block2synthon[1], block2rgroup[1]]
            )
            ...
            synthon_feats = self.factorizer.synthon_graph_encoder(synthons)
            ...
            # Embed the products
            with torch.no_grad():
                embeds: Tensor = self.encoder(products)

            # Get APEX predicted embeddings
            apex_embeds: Tensor = (
                self.factorizer.embed_predictor_pooling_function(
                    library_tensors["synthon_associative_embeds"][
                        block2asynthon
                    ],
                    block2product,
            ...
            if checkpoint_iterations is not None:
                if (iteration % checkpoint_iterations == 0) or (
                    ...
                    state_dict = self._get_checkpoint_state(
                        iteration, loss, optimizer
                    )

                    logger.debug(f"Check-pointing model iteration {iteration}")
